  ...
  return (
    <header className='bg-[#121216f6] w-full p-4 sm:flex justify-between'>
      ...
      <nav className={`w-full sm:2/3 lg:w-2/4 ${flag?'block':'hidden sm:flex justify-between'} text-[#fff] text-left py-2`}>
        <p className='border-b text-base sm:text-lg md:text-xl py-0.5 sm:border-none font-poppins'>
          <NavLink to="/">Home</NavLink>
        </p>
        <p className='border-b text-base sm:text-lg md:text-xl py-0.5 sm:border-none font-poppins'>
          <NavLink to="/games">Browse</NavLink>
        </p>
        <p className='border-b text-base sm:text-lg md:text-xl py-0.5 sm:border-none font-poppins relative'>
          <NavLink to="/downloads">
            Downloads{data.length!==0&&<span className='bg-sky-400 text-black text-sm w-5 h-5 text-center rounded-full absolute -top-1'>{data.length}</span>}
          </NavLink>
          </p>
