...
export const Poster = ({name,thumbMobile,thumbPc,description}:propsType) => {
    ...
    const showImages = () => {
        ...
        <picture>
            ...
            <div className={`absolute inset-0 bg-gradient-to-t from-black/70 ${!hideImage?"block":'hidden'}`}></div>
    ...
    return<div className='poster w-[95x%] sm:w-[100%] mx-auto relative snap-start'>
        {hideImage&&<div className='w-[100%] bg-slate-800 aspect-[2/3] sm:aspect-[5/2] md:aspect-[4/2] animate-pulse'></div>}
        {showImages()}
        </div>
}